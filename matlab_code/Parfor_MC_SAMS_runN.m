function Parfor_MC_SAMS_runN(~, M_value, cohr_flag, ...
...
if ~cohr_flag
    figpath = ['SAM_ML_N_' timestamp '/ParFor_indp_M' num2str(M_value) '_L' num2str(Large_Scale_Flag) '/' ];
else 
    figpath = ['SAM_ML_N_' timestamp '/ParFor_cohr_M' num2str(M_value) '_L' num2str(Large_Scale_Flag) '/' ];
end
...
CRBlist = zeros(1, length(VEC_N)); % row vec

%%
PowerDOAdB = [3;4];%[5; 3]; % in dB
DOA = [35.11 45.15]; % true DOA angles, off gird case
%%
for i_N = 1:length(VEC_N)
    N = VEC_N(i_N);
...
%     for idx_MC = 1: Total_Monte_Tials
        [SE_all,  ~] = ...
            Compute_algos_StdErr(SNR, N, M_value, cohr_flag, PowerDOAdB, DOA);
...
%         SE_sum  = SE_sum   + SE_all_m;
    ...
    for idx = 1: Num_algos
        ...
        SE_mean(i_N, idx) = mean(  SE_history_sorted(idx, count_pointer:end) );
        
        
        Failing_rate(i_N, idx) = Failed_total_times(idx) /Total_Monte_Tials;
    end
    ...
    CRBlist(i_N) = SAM_CRB(SNR, N, cohr_flag, PowerDOAdB, DOA);
...
for ind = ids_selected_algos %1: Num_algos
    ...
    semilogy(VEC_N, prepare, colorSet{ind},'DisplayName',algo_list{ind});
    hold on;
end

% -----CRB ----
semilogy(VEC_N, sqrt(CRBlist), colorSet{ind+1},'DisplayName','CRB'); 
% lgd_list = [algo_list, 'CRB'];

xlabel('N','Interpreter','latex');
ylabel('Angle RMSE (degree)');
if ~cohr_flag
    title(['Indpendent M=' num2str(M_value) ', SNR=' num2str(SNR)]);
else
    title(['Coherent M=' num2str(M_value) ', SNR=' num2str(SNR)]);
end
