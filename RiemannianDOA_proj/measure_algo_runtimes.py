...
from utils import get_doa_grid, get_algo_dict_list, Config
...
def measure_algo_times():
    ...
    for ii, m in enumerate(vec_m):
        N = int(3 * m)
        config = Config(m=m, snr=snr, N=N, power_doa_db=power_doa_db, doa=doa)
        ...
        for jj in range(NUM_TIMES):
            ...
            _, _, p_vec_cell, runtime_list, num_iters_list = run_single_mc_iteration(
                i_mc=jj, algo_list=list(algo_list.keys()), config=config, A_true=A_true, A=A,
                doa_scan=doa_scan, seed=jj
            )
